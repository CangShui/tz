...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/CangShui/ServerStatus-Hotaru/master/status.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	wget -N --no-check-certificate "https://raw.githubusercontent.com/CangShui/ServerStatus-Hotaru/master/status.sh" && chmod +x status.sh
...
echo && echo -e "  ServerStatus 一键安装管理脚本 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
 ...
 ${Green_font_prefix} 0.${Font_color_suffix} 升级脚本
 ————————————
 ${Green_font_prefix} 1.${Font_color_suffix} 安 服务端
 ${Green_font_prefix} 2.${Font_color_suffix} 更 服务端
 ${Green_font_prefix} 3.${Font_color_suffix} 卸 服务端
————————————
